...
public:
    ...
    bool SetHue(){
        if(Mouse::Get().GetLeftButton().is_held && ufoMaths::RectangleVsPoint(ufo::Rectangle(hue_slider_sprite_ref->GetGlobalPosition(), hue_slider_sprite_ref->frame_size) , Mouse::Get().GetPosition())){
    ...
    Colour GetColour(Vector2f _mouse_position){
        if(ufoMaths::RectangleVsPoint(ufo::Rectangle(colour_picker_sprite_ref->GetGlobalPosition(), colour_picker_sprite_ref->frame_size) , _mouse_position)){
